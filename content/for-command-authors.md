...
*Audience: you build the commands that [Users](For-Command-Users) compose.*

The rule of authoring: **pick a pattern from [`patterns/`](https://github.com/gloo-foo/framework/tree/main/patterns) and supply only your algorithm.** The pattern owns every hard part — stream wiring, channels, backpressure, cancellation, teardown — and hands you a plain function to fill in. You write *what the command does to a line*, never *how lines move*.
...
- **Need to see *all* input first** (sort, reverse, tail)? `Accumulate` (same type out) or `Aggregate` (one summary out).
...
- **`GenerateFrom(ctx, in, producer)`** — produce output derived from an input stream. A downstream stop tears down both your producer and the upstream `in`, because you pass the *stream*, not a loose handle.
